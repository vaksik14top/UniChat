        ...
        <div className={`app-layout ${chatId ? 'chat-open' : ''}`}>
                ...
                {showSettings && (
                    <div className="settings-panel settings-window">
                        ...
                        <div className="settings-item settings-toggle-row">
                            <span className="settings-item-label">{notificationsEnabled ? '🔔 Уведомления включены' : '🔕 Уведомления выключены'}</span>
                            <button className={`btn btn-secondary notification-toggle ${notificationsEnabled ? 'active' : ''}`} onClick={toggleNotifications}>
            ...
            {showChatMenu && chatId && <div className="modal-overlay" onClick={() => setShowChatMenu(false)}>
                <div className="modal chat-actions-modal" onClick={(event) => event.stopPropagation()}>
                    ...
                    {!isChannel && <button className="chat-action-row" onClick={toggleChatMute}>🔇 <span>{isChatMuted ? 'Включить звук' : 'Выключить звук'}</span></button>}
                    ...
                    <button className="chat-action-row" onClick={leaveCurrentChat}>↩ <span>{activeChat?.type === 'CHANNEL' ? 'Выйти из канала' : 'Выйти из чата'}</span></button>
                    {canDeleteActiveChat && <button className="chat-action-row danger-action" onClick={deleteCurrentChat}>⌫ <span>{activeChat?.type === 'CHANNEL' ? 'Удалить канал' : 'Удалить чат'}</span></button>}
                </div>
            </div>}
